...
READ_INTERVAL = 0.05
BLOCK_SIZE = 4  # Make blocks 4 times bigger
# ---------------------------------------
...
def send_garbage(amount):
    ...
    try:
        # Create a unique file with the amount in the name
        unique_id = str(uuid.uuid4())[:8]
        filename = f"garbage_{amount}_lines_{unique_id}.txt"
    ...
    try:
        ...
        for fname in files:
            if fname.startswith("garbage_") and fname.endswith(".txt"):
                # Extract the number of lines from the filename
                try:
                    parts = fname.split("_")
                    lines = int(parts[1])
                    garbage_count += lines
    ...
    try:
        ...
        if shape:
            ...
            for r in range(len(shape)):
                for c in range(len(shape[0])):
                    if shape[r][c]:
                        block_str = "█" * BLOCK_SIZE
                        stdscr.addstr(row_offset + y_offset + r, x_offset + c * BLOCK_SIZE, block_str)
    except curses.error:
        pass
        ...
        for r in range(HEIGHT):
            ...
            for c in range(WIDTH):
                if board[r][c]:
                    ...
                    block_str = "█" * BLOCK_SIZE
                    stdscr.addstr(offset_y + r + 1, offset_x + c * BLOCK_SIZE + 1, block_str, curses.color_pair(cell_color))
                else:
                    stdscr.addstr(offset_y + r + 1, offset_x + c * BLOCK_SIZE + 1, " " * BLOCK_SIZE)
        ...
        for r in range(len(shape)):
            for c in range(len(shape[0])):
                if shape[r][c] and y+r >= 0 and y+r < HEIGHT:
                    block_str = "█" * BLOCK_SIZE
                    stdscr.addstr(offset_y + y + r + 1, offset_x + (x + c) * BLOCK_SIZE + 1, block_str, curses.color_pair(color))
