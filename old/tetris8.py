...
READ_INTERVAL = 0.05
LEADERBOARD_READ_INTERVAL = 1.0  # Read leaderboard every second
...
last_read_time = 0
last_leaderboard_read_time = 0
pending_garbage = 0
last_clear_was_line = False  # Track if previous drop cleared a line
cached_leaderboard = []  # Cache leaderboard to avoid reading every frame
...
def save_score(name, lines_cleared):
    """Save score to a file in SHARED_DIR upon death"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        unique_id = str(uuid.uuid4())[:8]
        filename = f"score_{name}_{lines_cleared}_{timestamp}_{unique_id}.txt"
        filepath = f"{SHARED_DIR}/{filename}"
        open(filepath, "w").close()
    except:
        pass
...
def load_leaderboard():
    """Load leaderboard from score files in SHARED_DIR"""
    leaderboard = []
    try:
        files = os.listdir(SHARED_DIR)
        for fname in files:
            if fname.startswith("score_") and fname.endswith(".txt"):
                try:
                    # Format: score_{name}_{lines}_{timestamp}_{unique_id}.txt
                    parts = fname[:-4].split("_")  # Remove .txt
                    if len(parts) >= 4:
                        name = parts[1]
    ...
    # Sort by lines cleared (descending) and keep top 10
    leaderboard.sort(key=lambda x: x["lines"], reverse=True)
    return leaderboard[:10]

def display_leaderboard_final(stdscr, player_lines):
...
def draw_leaderboard(stdscr, leaderboard, row_offset, col_offset):
    """Draw the live leaderboard on the side"""
    try:
        stdscr.addstr(row_offset, col_offset, "TOP 10 SCORES", curses.A_BOLD)
        stdscr.addstr(row_offset + 1, col_offset, "-" * 20)
        
        if not leaderboard:
            stdscr.addstr(row_offset + 2, col_offset, "No scores yet!")
        else:
            for i, entry in enumerate(leaderboard[:10]):
                row = row_offset + 2 + i
                # Truncate name if too long
                name = entry['name'][:8]
                line_text = f"{i+1:>2}. {name:<8} {entry['lines']:>4}L"
                
                # Highlight current player
                if entry['name'] == PLAYER:
                    stdscr.addstr(row, col_offset, line_text, curses.A_BOLD | curses.color_pair(4))
                else:
                    stdscr.addstr(row, col_offset, line_text)
    except curses.error:
        pass
...
def draw(stdscr, board, shape, piece_name, x, y, garbage_pending, next_shape, next_piece_name, 
    ...
    preview_width = 14
    leaderboard_width = 22
    total_width = leaderboard_width + 2 + field_width + preview_width + 2
    ...
    # Leaderboard column (left side)
    leaderboard_col = offset_x
    
    # Game board column (after leaderboard)
    board_col = offset_x + leaderboard_width + 2
    
    # Preview column (right side)
    preview_col = board_col + field_width + 2
    ...
    try:
        ...
        draw_leaderboard(stdscr, leaderboard, offset_y, leaderboard_col)
        
        # Draw top border
        top_border = "#" * field_width
        stdscr.addstr(offset_y, board_col, top_border)
        
        # Draw board with side walls
        for r in range(HEIGHT):
            stdscr.addstr(offset_y + r + 1, board_col, "#")
            for c in range(WIDTH):
                    ...
                    for pc in range(len(shape[0])):
                        if shape[pr][pc]:
                            if r == y + pr and c == x + pc:
                                stdscr.addstr(offset_y + r + 1, board_col + c * BLOCK_SIZE + 1, "[]", curses.color_pair(color))
                                cell_drawn = True
                                break
                    if cell_drawn:
                        break
                        ...
                        for pc in range(len(shape[0])):
                            if shape[pr][pc]:
                                if r == ghost_y + pr and c == x + pc:
                                    stdscr.addstr(offset_y + r + 1, board_col + c * BLOCK_SIZE + 1, "..", curses.color_pair(color) | curses.A_DIM)
                                    cell_drawn = True
                                    break
                        if cell_drawn:
                            break
                ...
                if not cell_drawn:
                    if board[r][c]:
                        ...
                        stdscr.addstr(offset_y + r + 1, board_col + c * BLOCK_SIZE + 1, "[]", curses.color_pair(cell_color))
                    else:
                        stdscr.addstr(offset_y + r + 1, board_col + c * BLOCK_SIZE + 1, "  ")
                
            stdscr.addstr(offset_y + r + 1, board_col + field_width - 1, "#")
        
        # Draw bottom border
        stdscr.addstr(offset_y + HEIGHT + 1, board_col, top_border)
        
        # Draw preview boxes on the right
        ...
        stdscr.addstr(info_y, board_col, f"Player: {PLAYER}    Lines: {total_lines}         ")
        
        if garbage_pending > 0:
            stdscr.addstr(info_y + 1, board_col, f"Garbage incoming: {garbage_pending}    ")
        else:
            stdscr.addstr(info_y + 1, board_col, " " * 30)
        
        if last_rotation_was_tspin:
            stdscr.addstr(info_y + 2, board_col, "T-SPIN!                    ", curses.A_BOLD | curses.color_pair(3))
        else:
            stdscr.addstr(info_y + 2, board_col, " " * 30)
        
        controls = "A/D=Move W=Rotate S=Soft SPACE=Hard "
        controls += "C=Hold(used) " if not can_hold else "C=Hold "
        controls += "Q=Quit"
        stdscr.addstr(info_y + 3, board_col, controls)
...
def main(stdscr):
    global pending_garbage, last_read_time, last_clear_was_line, last_leaderboard_read_time, cached_leaderboard
    ...
    cached_leaderboard = load_leaderboard()
    ...
    while True:
        ...
        # Periodically refresh leaderboard cache
        if current_time - last_leaderboard_read_time >= LEADERBOARD_READ_INTERVAL:
            last_leaderboard_read_time = current_time
            cached_leaderboard = load_leaderboard()
        ...
        if key == ord('q'):
            ...
            save_score(PLAYER, total_lines)
            time.sleep(0.1)  # Small delay to ensure file is written
            display_leaderboard_final(stdscr, total_lines)
            break
        ...
        draw(stdscr, board, shape, current_piece_name, x, y, pending_garbage, 
             ...
             can_hold, current_color, last_rotation_was_tspin, total_lines, cached_leaderboard)
